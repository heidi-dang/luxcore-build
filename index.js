 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`luxcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`luxcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'luxcore-' + name : 'luxcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/luxcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external luxcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:luxcore-lib -o luxcore-lib.js';
    }
